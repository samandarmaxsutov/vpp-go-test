...
get_current_policy_epoch (acl_main_t *am, int is_input, u32 sw_if_index0)
{
  ...
  u16 current_policy_epoch = sw_if_index0 < vec_len (*p_epoch_vec) ?
			       vec_elt (*p_epoch_vec, sw_if_index0) :
		       ...
		       fa_full_session_id_t f_sess_id)
{
  ...
  /* if the MSB of policy epoch matches but not the LSB means it is a stale
   * session */
  if ((0 == ((current_policy_epoch ^ f_sess_id.intf_policy_epoch) &
		...
		fa_5tuple_t *out_fa_5tuple)
{
  int ii;
  for (ii = 0; ii < vector_sz; ii++)
    acl_fill_5tuple (am, sw_if_index[ii], b[ii], is_ip6, is_input, is_l2_path,
		     &out_fa_5tuple[ii]);
}
			     ...
			     int node_trace_on, u32 *trace_bitmap)
{
  ...
  action = acl_fa_track_session (am, is_input, sw_if_index[0], now, sess,
				 &fa_5tuple[0], pkt_len);
  ...
  if (PREDICT_FALSE (old_timeout_type != new_timeout_type))
    {
      ...
      if (node_trace_on)
	*trace_bitmap |= 0x00010000 + ((0xff & old_timeout_type) << 8) +
		      ...
		      int node_trace_on, int reclassify_sessions)
{
  ...
  while (n_left > 0)
      ...
      if (with_stateful_datapath)
	{
	  ...
	  switch (n_left)
	      ...
	      if (f_sess_id.as_u64 != ~0ULL)
		{
		  ...
		  action = process_established_session (
		    vm, am, node->node_index, is_input, now, f_sess_id,
		    &sw_if_index[0], &fa_5tuple[0], b[0]->current_length,
		  ...
		  if (reclassify_sessions)
		    {
		      if (PREDICT_FALSE (stale_session_deleted (
			    am, is_input, pw, now, sw_if_index[0], f_sess_id)))
			{
			   ...
			   * buffer is the same 5-tuple, that session
			   * prediction is wrong, correct it.
			   */
			  if ((f_sess_id_next.as_u64 != ~0ULL) &&
	  ...
	  if (acl_check_needed)
		...
		{
		  /* Faqat drop (action=0) bo'lganlarni log qilish uchun */
		  if (action == 0)
		    {
		      static f64 last_log_time = 0;
		      f64 now_vlib = vlib_time_now (vm);

		      if (now_vlib - last_log_time > 1.0)
			{
			  vlib_log_warn (am->log_default,
					 "ACL_DROP: interface:%d, action:%d, "
					 "match: acl_idx:%d rule_idx:%d",
					 sw_if_index[0], action,
					 match_acl_in_index, match_rule_index);

			  u8 *s = 0;
			  s = format (s, "Dropped Pkt Details: %U",
				      format_fa_5tuple, &fa_5tuple[0]);
			  vlib_log_warn (am->log_default, "%v", s);

			  vec_free (s);
			  last_log_time = now_vlib;
			}
		    }
		}
	      if (PREDICT_FALSE (is_match &&
...

		{
		  ...
		  vlib_increment_combined_counter (
		    ...
		    thread_index, saved_matched_ace_index, saved_packet_count,
		    saved_byte_count);
	      ...
	      if (2 == action)
		{
		  ...
		  if (acl_fa_can_add_session (am, is_input, sw_if_index[0]))
		    {
		      u16 current_policy_epoch = get_current_policy_epoch (
			am, is_input, sw_if_index[0]);
		      fa_full_session_id_t f_sess_id = acl_fa_add_session (
			am, is_input, is_ip6, sw_if_index[0], now,
			&fa_5tuple[0], current_policy_epoch);
			...
			node_trace_on, &trace_bitmap);
		      pkts_new_session++;
		      /*
		       * If the next 5tuple is the same and we just added the
		       * session, the f_sess_id_next can not be ~0. Correct it.
		       */
		      if ((f_sess_id_next.as_u64 == ~0ULL) &&
			  0 == memcmp (&fa_5tuple[1], &fa_5tuple[0],
				       sizeof (fa_5tuple[1])))
	  ...
	  if (node_trace_on) // PREDICT_FALSE (node->flags &
  ...
  vlib_node_increment_counter (vm, node->node_index, ACL_FA_ERROR_ACL_CHECK,
			       frame->n_vectors);
  vlib_node_increment_counter (
    vm, node->node_index, ACL_FA_ERROR_ACL_EXIST_SESSION, pkts_exist_session);
  vlib_node_increment_counter (vm, node->node_index,
			       ACL_FA_ERROR_ACL_NEW_SESSION, pkts_new_session);
  vlib_node_increment_counter (vm, node->node_index, ACL_FA_ERROR_ACL_PERMIT,
			       pkts_acl_permit);
		...
		vlib_frame_t *frame, int is_ip6, int is_input, int is_l2_path)
{
  ...
  if (am->fa_sessions_hash_is_initialized)
    rv =
      acl_fa_outer_node_fn (vm, node, frame, is_ip6, is_input, is_l2_path, 1);
  else
    rv =
      acl_fa_outer_node_fn (vm, node, frame, is_ip6, is_input, is_l2_path, 0);
...
VLIB_NODE_FN (acl_in_l2_ip6_node)
...
VLIB_NODE_FN (acl_out_l2_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return acl_fa_node_fn (vm, node, frame, 1, 0, 1);
}

VLIB_NODE_FN (acl_out_l2_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
VLIB_NODE_FN (acl_in_fa_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return acl_fa_node_fn (vm, node, frame, 1, 1, 0);
}

VLIB_NODE_FN (acl_in_fa_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
VLIB_NODE_FN (acl_out_fa_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
VLIB_REGISTER_NODE (
  acl_in_l2_ip6_node) = { .name = "acl-plugin-in-ip6-l2",
...
VLIB_REGISTER_NODE (
  acl_out_l2_ip6_node) = { .name = "acl-plugin-out-ip6-l2",
			   .vector_size = sizeof (u32),
			   .format_trace = format_acl_plugin_trace,
			   .type = VLIB_NODE_TYPE_INTERNAL,
			   .n_errors = ARRAY_LEN (acl_fa_error_strings),
...
VLIB_REGISTER_NODE (
  acl_out_l2_ip4_node) = { .name = "acl-plugin-out-ip4-l2",
			   .vector_size = sizeof (u32),
			   .format_trace = format_acl_plugin_trace,
			   .type = VLIB_NODE_TYPE_INTERNAL,
			   .n_errors = ARRAY_LEN (acl_fa_error_strings),
...
VLIB_REGISTER_NODE (
  acl_in_fa_ip6_node) = { .name = "acl-plugin-in-ip6-fa",
			  .vector_size = sizeof (u32),
			  .format_trace = format_acl_plugin_trace,
			  .type = VLIB_NODE_TYPE_INTERNAL,
			  .n_errors = ARRAY_LEN (acl_fa_error_strings),
...
VLIB_REGISTER_NODE (
  acl_in_fa_ip4_node) = { .name = "acl-plugin-in-ip4-fa",
			  .vector_size = sizeof (u32),
			  .format_trace = format_acl_plugin_trace,
			  .type = VLIB_NODE_TYPE_INTERNAL,
			  .n_errors = ARRAY_LEN (acl_fa_error_strings),
...
VLIB_REGISTER_NODE (
  acl_out_fa_ip4_node) = { .name = "acl-plugin-out-ip4-fa",
			   .vector_size = sizeof (u32),
			   .format_trace = format_acl_plugin_trace,
			   .type = VLIB_NODE_TYPE_INTERNAL,
			   .n_errors = ARRAY_LEN (acl_fa_error_strings),
